...
distilled_from:
  ...
  - "[[2023-lipman-flow-matching]]"
---

# Neural optimal transport

Parameterize the OT solution with **neural networks** — typically the **dual potentials**, a
**stochastic transport map**, or a **CNF vector field** — so the solution applies to continuous,
high-dimensional measures and generalizes to unseen samples, instead of solving a fixed discrete
coupling.
...
essential for generative modeling, unpaired translation, and single-cell perturbation prediction.
...
- **Flow-matching / CNF approach:** regress a continuous vector field onto conditional OT
  displacement paths (or diffusion paths) with Conditional Flow Matching — simulation-free CNF
  training that scales to ImageNet ([[2023-lipman-flow-matching]]).
- These rest on OT **duality** or **displacement interpolation**
  ([[monge-kantorovich-formulations]]); stability comes from architectural inductive biases
  (convexity constraints), saddle-point schedules, or straight OT path targets.
...
- introduces [[2023-lipman-flow-matching]] — Flow Matching + OT displacement paths as CNF transport
- extends [[monge-kantorovich-formulations]] — a neural parameterization of the OT dual / flow

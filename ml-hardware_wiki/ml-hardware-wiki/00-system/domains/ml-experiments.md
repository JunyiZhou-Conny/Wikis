...
When logging papers or course notes that inform experiments, emit `type: source` notes under
the matching subfolder (`sources/papers/`, `sources/courses/`, …). See
[`literature-sources.md`](literature-sources.md) for paper frontmatter. Link runs to sources
via `source_refs` or wikilinks in the run note body.
